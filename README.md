...
Um diário de casal interativo em 3D de verdade: capa de tecido quadriculado
azul com espessura real, luz de sol projetando sombra, você pega e arrasta a
capa/páginas fisicamente com a física de uma dobradiça de verdade — e por
dentro dá pra montar as páginas livremente (tipo Canva) — texto, foto, áudio
e vídeo — com contador de tempo juntos.

## Estrutura do projeto

```
index.html          → página única do site
style.css            → aparência do conteúdo de página + UI 2D (título, nav, modais)
...
js/editor.js         → constrói o HTML de cada página (arrastar/redimensionar elementos)
js/scene3d.js         → a cena 3D em si: câmera, luz, grama, física de abrir/virar (Three.js)
js/effects.js         → som "boof" sintetizado (WebAudio)
js/app.js             → liga tudo (cena 3D, navegação, contador, modais)
vendor/three/         → Three.js vendorizado localmente (sem depender de CDN externo)
...
- **Ver o diário:** o cursor vira uma "mãozinha" ao passar sobre a capa —
  clique e arraste da direita pra esquerda pra puxar e abrir (ou só clique).
  Pra virar página, pegue o canto direito dela e arraste do mesmo jeito. Use
  as setas embaixo (ou ← →) como atalho, e o × pra fechar o diário.
...
## Placeholder visual — texturas que faltam pro resultado final

A cena hoje usa cores chapadas com sombreamento "cel-shading" (estilo
low-poly/toon) como placeholder. Pra chegar no visual final, ver a lista
completa de arquivos de textura sugeridos no comentário no fim de
`js/scene3d.js` — nenhum é obrigatório, mas cada um eleva o acabamento:
tecido/couro da capa, papel das páginas, grama de verdade, e um HDRI de céu
opcional pra iluminação ainda mais realista.
